...
git pull
echo --BENCH-CREATE--
python3.7 -m mlprodict asv_bench --location . --runtime "scikit-learn,python,onnxruntime1" --conf_params "project,asv-skl2onnx;project_url,https://github.com/sdpython/asv-skl2onnx" --models SVMRegressor,SVMClassifier,RandomForestRegressor,RandomForestClassifier,DecisionTreeRegressor,DecisionTreeClassifier,AdaBoostClassifier,AdaBoostRegressor -v 1 || exit 1
echo --BENCH-RUN--
python3.7 -m asv run --show-stderr --config asv.conf.json --environment existing:same
if [ -d html ]
then
    echo --REMOVE HTML--
    rm html -r -f
